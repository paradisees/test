...
for file_num in range(10):
    ...
    mark = np.array(mark)
    x_test2, x_test3, y_test2, y_test3 = cross_validation.train_test_split(
        data, mark, test_size=0.05, random_state=file_num)
    ...
    # 给x，y留出占位符，以便未来填充数据
    x = tf.placeholder("float", [None, 712])
    y_ = tf.placeholder("float", [None, 1])
    # 设置输入层的W和b
    W = tf.Variable(tf.zeros([712, 1]))
    ...
    print(h_pool2.shape)
    # 全连接层的w和b
    w_fc1 = weight_variable([2*23*64, 712])
    b_fc1 = bias_variable([712])
    # 此时输出的维数是256维
    h_pool2_flat = tf.reshape(h_pool2, [-1, 2*23*64])
    ...
    print("test accuracy is %g" % accuracy.eval(feed_dict={x: x_test3, y_: y_test3, keep_prob: 1.0}))
    # def my_test(input_x):
    ...
    x_temp = []
    for g in x_test2:
        x_temp.append(sess.run(h_fc1, feed_dict={x: np.array(g).reshape((1, 712))})[0])
    # 将原来的x带入训练好的CNN中计算出来全连接层的特征向量，将结果作为SVM中的特征向量
    x_temp2 = []
    for g in x_test3:
        x_temp2.append(sess.run(h_fc1, feed_dict={x: np.array(g).reshape((1, 712))})[0])
    ...
    for j in range(len(x_temp2)):
        ...
        if clf.predict(x_temp2[j])[0] == y_test3[j] == 1:
            ...
            error1 += 1
    res.append(clf.score(x_temp2, y_test3))
...
print('recall ', recall)
print('F1 score ', 2 * accuracy * recall / (accuracy + recall))
print(res)