...
X, X_predict, y, y_predict = train_test_split(X, target, test_size=0.05, random_state=3)
#第二层模型的训练和测试集
...
over_time=time.time()
used_time=over_time-initial_time
print('used time: %f' % round(used_time,3))
y_pred = clf.predict(dataset_blend_test)
print("blend result val acc Score: %f" % (metrics.accuracy_score(y_predict,y_pred)))
print("blend result val f1 Score: %f" % (metrics.f1_score(y_predict, y_pred)))
